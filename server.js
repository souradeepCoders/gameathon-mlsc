...
const matchSchema = new mongoose.Schema({
    gameName: String,
    teamA: String, // We will now store "PlayerName (TeamName)" here
    teamB: String
});
...
app.post('/register', async (req, res) => {
    try {
        ...
        res.status(400).json({ error: "Registration failed. Team ID might already exist." });
    }
});
...
app.post('/end-match', async (req, res) => {
    try {
        ...
        res.json({ message: "Match ended. Don't forget to award points!" });
    } catch (error) {
...
// Keep local hosting working for testing
